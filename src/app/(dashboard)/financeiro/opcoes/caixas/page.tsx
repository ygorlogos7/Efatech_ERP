...
import { getCaixas, createCaixa } from "@/actions/financeiro";
import { PiggyBank, PlusCircle, Check, X } from "lucide-react";
...
export default function CaixasPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const r = await getCaixas();
      if (r.success) setItems(r.data as any[]);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const r = await createCaixa(formData);
    if (r.success) {
      setIsAdding(false);
      const u = await getCaixas();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Caixas</h2>
        <button
        ...
        >
          <PlusCircle className="w-4 h-4" /> Novo Caixa
        </button>
      </div>
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-4 items-end animate-in slide-in-from-top duration-200">
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Nome do Caixa *</label>
            <input type="text" name="Nome" required placeholder="Ex: Caixa Principal" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
          <div>
