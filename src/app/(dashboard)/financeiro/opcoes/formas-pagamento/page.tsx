...
import { getFormasPagamento, createFormaPagamento } from "@/actions/financeiro";
...
export default function FormasPagamentoPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const r = await getFormasPagamento();
      if (r.success) setItems(r.data as any[]);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const r = await createFormaPagamento(formData);
    if (r.success) {
      setIsAdding(false);
      const u = await getFormasPagamento();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Formas de Pagamento</h2>
        <button
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 flex items-end gap-4 animate-in slide-in-from-top duration-200">
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-700 mb-1">Nome da Forma *</label>
            <input type="text" name="Nome" required placeholder="Ex: Cartão de Crédito, PIX..." className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
