...
import { getCentrosCusto, createCentroCusto } from "@/actions/financeiro";
import { Network, PlusCircle, Check, X } from "lucide-react";
...
export default function CentrosCustoPage() {
  ...
  const handleAdd = async (formData: FormData) => {
    const r = await createCentroCusto(formData);
    if (r.success) {
      setIsAdding(false);
      const u = await getCentrosCusto();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Centros de Custos</h2>
        <button
        ...
        >
          <PlusCircle className="w-4 h-4" /> Novo Centro
        </button>
      </div>
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 flex items-end gap-4 animate-in slide-in-from-top duration-200">
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-700 mb-1">Nome do Centro *</label>
            <input type="text" name="Nome" required placeholder="Ex: Administrativo, Vendas..." className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
