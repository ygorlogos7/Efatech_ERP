...
import { getContasBancarias, createContaBancaria } from "@/actions/financeiro";
import { Landmark, PlusCircle, Check, X } from "lucide-react";
...
export default function ContasBancariasPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const r = await getContasBancarias();
      if (r.success) setItems(r.data as any[]);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const r = await createContaBancaria(formData);
    if (r.success) {
      setIsAdding(false);
      const u = await getContasBancarias();
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-end animate-in slide-in-from-top duration-200">
          <div>
            ...
            <input type="text" name="Banco" required placeholder="Ex: Itaú" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Titular</label>
            <input type="text" name="Titular" placeholder="Ex: Empresa Ltda" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Saldo Inicial (R$)</label>
            <input type="number" name="SaldoInicial" step="0.01" defaultValue="0" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
      ...
      <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-100">
        <table className="w-full text-sm text-left border-collapse min-w-[700px]">
          <thead className="bg-[#f8f9fa] border-b border-gray-200 text-gray-700 font-semibold">
            <tr>
              <th className="py-3 px-6">Banco</th>
              <th className="py-3 px-6">Titular</th>
              <th className="py-3 px-6 text-right">Saldo Atual</th>
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              <tr key={item.Id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="py-3 px-6 font-bold text-gray-900">{item.Banco}</td>
                <td className="py-3 px-6 text-gray-700">{item.Titular || "-"}</td>
                <td className="py-3 px-6 text-right font-bold text-green-700">R$ {item.SaldoAtual.toFixed(2).replace(".", ",")}</td>
