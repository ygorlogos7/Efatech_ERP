...
import { getMovimentacoes, createMovimentacao } from "@/actions/estoque";
import { Package, PlusCircle, ArrowUp, ArrowDown, Check, X } from "lucide-react";
...
export default function MovimentacoesPage() {
  ...
  React.useEffect(() => { startTransition(async () => { const r = await getMovimentacoes(); if (r.success) setItems(r.data as any[]); }); }, []);

  const handleAdd = async (formData: FormData) => {
    const r = await createMovimentacao(formData);
    if (r.success) { setIsAdding(false); const u = await getMovimentacoes(); if (u.success) setItems(u.data as any[]); }
  };

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Movimentações de Estoque</h2>
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-end animate-in slide-in-from-top duration-200">
          ...
          <div>
            ...
            <input type="text" name="Motivo" placeholder="Ex: Compra, Venda, Perda..." className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
      ...
      <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-100">
        <table className="w-full text-sm text-left border-collapse min-w-[600px]">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={4} className="text-center py-16 text-gray-500"><Package className="w-10 h-10 mx-auto mb-2 opacity-30"/><p>Nenhuma movimentação registrada.</p></td></tr>
            ) : items.map(item => (
              <tr key={item.Id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="py-3 px-4 text-center">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold rounded border ${item.Tipo === "entrada" ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-600 border-red-200"}`}>
                    {item.Tipo === "entrada" ? <ArrowDown className="w-3 h-3"/> : <ArrowUp className="w-3 h-3"/>}
                    {item.Tipo === "entrada" ? "Entrada" : "Saída"}
                  </span>
                </td>
                <td className="py-3 px-6 text-gray-700">{item.Motivo || "-"}</td>
                <td className="py-3 px-4 text-right font-bold">{item.Quantidade}</td>
                <td className="py-3 px-4 text-gray-500">{new Date(item.DataMovimento).toLocaleDateString("pt-BR")}</td>
              </tr>
            ))}
          </tbody>
        </table>
