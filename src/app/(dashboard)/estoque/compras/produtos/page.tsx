...
import { getComprasProdutos, createCompraProdutos } from "@/actions/estoque";
import { Package, PlusCircle, Check, X } from "lucide-react";
...
export default function ComprasProdutosPage() {
  const [items, setItems] = useState<any[]>([]);
  const [isAdding, setIsAdding] = useState(false);
  const [isPending, startTransition] = useTransition();

  React.useEffect(() => { startTransition(async () => { const r = await getComprasProdutos(); if (r.success) setItems(r.data as any[]); }); }, []);

  const handleAdd = async (formData: FormData) => {
    const r = await createCompraProdutos(formData);
    if (r.success) { setIsAdding(false); const u = await getComprasProdutos(); if (u.success) setItems(u.data as any[]); }
  };

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Compras — Produtos</h2>
      ...
      <div className="bg-white rounded-md shadow-sm overflow-hidden border border-gray-100">
        <table className="w-full text-sm text-left border-collapse min-w-[600px]">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-16 text-gray-500"><Package className="w-10 h-10 mx-auto mb-2 opacity-30"/><p>Nenhuma compra registrada.</p></td></tr>
            ) : items.map(item => (
