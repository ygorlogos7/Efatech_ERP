...
import { getEstoqueSituacoes, createEstoqueSituacao } from "@/actions/estoque";
import { ClipboardList, PlusCircle, Check, X } from "lucide-react";

export default function EstoqueSituacoesPage() {
  const [items, setItems] = useState<any[]>([]);
  const [isAdding, setIsAdding] = useState(false);
  const [isPending, startTransition] = useTransition();
  React.useEffect(() => { startTransition(async () => { const r = await getEstoqueSituacoes(); if (r.success) setItems(r.data); }); }, []);
  const handleAdd = async (fd: FormData) => { const r = await createEstoqueSituacao(fd); if (r.success) { setIsAdding(false); const u = await getEstoqueSituacoes(); if (u.success) setItems(u.data); } };
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Situações — Estoque</h2>
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-5 rounded-md shadow-sm border border-gray-100 flex items-end gap-4 animate-in slide-in-from-top duration-200">
          <div className="flex-1"><label className="block text-xs font-bold text-gray-700 mb-1">Nome *</label><input type="text" name="Nome" required placeholder="Ex: Em falta" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" /></div>
          <div className="w-[120px]"><label className="block text-xs font-bold text-gray-700 mb-1">Cor</label><input type="color" name="Cor" defaultValue="#f59e0b" className="w-full h-[34px] border border-gray-300 rounded px-1 py-0.5 cursor-pointer" /></div>
