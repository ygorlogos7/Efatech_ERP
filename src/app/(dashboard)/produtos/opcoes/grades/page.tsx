...
import { getProdutoGrades, createProdutoGrade } from "@/actions/produtosAux";
import { GitBranch, PlusCircle, Box } from "lucide-react";

export default function ProdutoGradesPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const res = await getProdutoGrades();
      if (res.success) setItems(res.data);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const res = await createProdutoGrade(formData);
    if (res.success) {
      setIsAdding(false);
      const updated = await getProdutoGrades();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-dark font-bold text-3xl text-gray-900 mb-0">Grades / Variações</h2>
        <button 
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-6 rounded-md shadow-sm border border-gray-100 flex items-end gap-4 animate-in slide-in-from-top duration-200">
          <div className="flex-1">
            <label className="block text-xs font-bold text-gray-700 mb-1">Nome da Grade (ex: Tamanho) *</label>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left align-middle border-collapse min-w-[600px]">
            <thead className="bg-[#f8f9fa] border-b border-gray-200 text-gray-700 font-semibold">
              <tr>
                <th className="py-3 px-6">Identificação da Grade</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={2} className="text-center py-16 text-gray-500">
                    ...
                    <h5 className="text-lg font-medium text-gray-700">Nenhuma grade cadastrada.</h5>
                  </td>
                </tr>
              ) : (
                items.map((item) => (
