...
import { getProdutoGrupos, createProdutoGrupo } from "@/actions/produtosAux";
import { Network, PlusCircle, Check, X, Box } from "lucide-react";
import { useFormStatus } from "react-dom";

export default function ProdutoGruposPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const res = await getProdutoGrupos();
      if (res.success) setItems(res.data);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const res = await createProdutoGrupo(formData);
    if (res.success) {
      setIsAdding(false);
      const updated = await getProdutoGrupos();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-dark font-bold text-3xl text-gray-900 mb-0">Grupos de Produtos</h2>
        <button 
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-6 rounded-md shadow-sm border border-gray-100 flex items-end gap-4 animate-in slide-in-from-top duration-200">
          <div className="flex-1">
            ...
            <input type="text" name="Nome" required required className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left align-middle border-collapse min-w-[600px]">
            <thead className="bg-[#f8f9fa] border-b border-gray-200 text-gray-700 font-semibold">
              <tr>
                <th className="py-3 px-6">Nome do Grupo</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={2} className="text-center py-16 text-gray-500">
                    ...
                    <h5 className="text-lg font-medium text-gray-700">Nenhum grupo cadastrado.</h5>
                  </td>
                </tr>
              ) : (
                items.map((item) => (
