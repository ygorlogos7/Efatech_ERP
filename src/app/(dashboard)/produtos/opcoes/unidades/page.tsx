...
import { getProdutoUnidades, createProdutoUnidade } from "@/actions/produtosAux";
import { FlaskConical, PlusCircle, Check, X, Box } from "lucide-react";

export default function ProdutoUnidadesPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const res = await getProdutoUnidades();
      if (res.success) setItems(res.data);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const res = await createProdutoUnidade(formData);
    if (res.success) {
      setIsAdding(false);
      const updated = await getProdutoUnidades();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-dark font-bold text-3xl text-gray-900 mb-0">Unidades de Produtos</h2>
        <button 
      ...
      {isAdding && (
        <form action={handleAdd} className="bg-white p-6 rounded-md shadow-sm border border-gray-100 flex items-end gap-4 animate-in slide-in-from-top duration-200">
          ...
          <div className="w-[100px]">
            <label className="block text-xs font-bold text-gray-700 mb-1">Sigla *</label>
            <input type="text" name="Sigla" required placeholder="Ex: KG" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left align-middle border-collapse min-w-[600px]">
            <thead className="bg-[#f8f9fa] border-b border-gray-200 text-gray-700 font-semibold">
              <tr>
                <th className="py-3 px-6">Nome da Unidade</th>
                <th className="py-3 px-6">Sigla</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={3} className="text-center py-16 text-gray-500">
                    ...
                    <h5 className="text-lg font-medium text-gray-700">Nenhuma unidade cadastrada.</h5>
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  <tr key={item.Id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-6 font-bold text-gray-700">{item.Sigla}</td>
