...
import { getOSConfig, saveOSConfig } from "@/actions/ordensServico";
...
export default function OSConfigPage() {
  ...
  const handleSave = async (formData: FormData) => {
    startTransition(async () => {
      const res = await saveOSConfig(formData);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Configurações de O.S.</h2>
        {saved && (
      ...
      <form action={handleSave} className="space-y-6 max-w-3xl">
        <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Prazo Padrão de Entrega (dias)</label>
              <input type="number" name="ValidadePadraoEmDias" defaultValue={config?.ValidadePadraoEmDias ?? 7} min="1" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">E-mail Padrão p/ Notificação</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-gray-700 mb-1">Texto Rodapé (impressão)</label>
              <textarea name="MensagemRodape" rows={3} defaultValue={config?.MensagemRodape || ""} placeholder="Garantia, termos de serviço..." className="w-full text-sm border border-gray-300 rounded p-3 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]"></textarea>
            </div>
            <div className="md:col-span-2">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm font-medium text-gray-700">Numeração automática das O.S.</span>
              </label>
            </div>
          </div>
        </div>
