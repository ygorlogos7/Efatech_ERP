...
import { getOSModelosEmail, createOSModeloEmail } from "@/actions/ordensServico";
import { Mail, PlusCircle, Check, X } from "lucide-react";

export default function OSModelosEmailPage() {
  ...
  React.useEffect(() => {
    startTransition(async () => {
      const res = await getOSModelosEmail();
      if (res.success) setItems(res.data);
    });
  }, []);

  const handleAdd = async (formData: FormData) => {
    const res = await createOSModeloEmail(formData);
    if (res.success) {
      setIsAdding(false);
      const updated = await getOSModelosEmail();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl font-bold text-gray-900">Modelos de E-mail — O.S.</h2>
        <button onClick={() => setIsAdding(!isAdding)} className="flex items-center gap-1.5 bg-[#00b050] hover:bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-md shadow-sm">
          <PlusCircle className="w-4 h-4" /> Adicionar Modelo
        </button>
      </div>
        ...
        <form action={handleAdd} className="bg-white p-6 rounded-md shadow-sm border border-gray-100 space-y-4 animate-in slide-in-from-top duration-200">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="Nome" required placeholder="Ex: O.S. finalizada" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Assunto *</label>
              <input type="text" name="Assunto" required placeholder="Ex: Sua O.S. está pronta!" className="w-full text-sm border border-gray-300 rounded px-3 py-1.5 focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859]" />
            </div>
          </div>
          <div>
          ...
          <div className="flex gap-2">
            <button type="submit" className="flex items-center gap-1.5 bg-[#00a65a] hover:bg-green-600 text-white font-bold py-2 px-5 rounded shadow-sm text-sm"><Check className="w-4 h-4" /> Salvar</button>
            <button type="button" onClick={() => setIsAdding(false)} className="flex items-center gap-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2 px-5 rounded shadow-sm text-sm"><X className="w-4 h-4" /> Cancelar</button>
          </div>
        </form>
      )}
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={3} className="text-center py-16 text-gray-500">
                ...
                <h5 className="text-lg font-medium text-gray-700">Nenhum modelo cadastrado.</h5>
              </td></tr>
            ) : items.map((item) => (
